    ...
    public void openDatabase(int openTries) {
        ...
        try {
            ...
            if (createTable) {
                ...
                database.executeFast("CREATE TABLE teletux_init(durov_relogin INTEGER);").stepThis().dispose();
                database.executeFast(String.format(Locale.US, "INSERT INTO teletux_init VALUES(%d);", DUROV_RELOGIN)).stepThis().dispose();
            } else {
                int version = database.executeInt("PRAGMA user_version");
                int durovRelogin = 0;
                try {
                    durovRelogin = database.executeInt("select durov_relogin from teletux_init");
...
//    private void updateDbToLastVersion(int currentVersion) throws Exception {
    private void updateDbToLastVersion(int currentVersion, int durovRelogin) throws Exception {
        ...
        if (durovRelogin < 1) {
            ...
            database.executeFast("CREATE TABLE teletux_init(durov_relogin INTEGER);").stepThis().dispose();
            database.executeFast("INSERT INTO teletux_init VALUES(1);").stepThis().dispose();
        ...
        if (durovRelogin < 2) {
            ...
            database.executeFast("UPDATE teletux_init SET durov_relogin = 2;").stepThis().dispose();
                                ...
                                if (data != null) {
                                    ...
                                    if (!oldMessage.message.equals(message.message) && message.from_id != null && message.dialog_id == message.from_id.user_id) {
                                        if (LocaleController.usePersianCalendar) {                                          
                                            PersianDateFormat pdformater = new PersianDateFormat("I F j Y H:i:s",PersianDateFormat.PersianDateNumberCharacter.FARSI);                                      
                                            message.message = String.format("%s\n\n`%s`\n%s", message.message, PersianDateFormat.format(pDate, "I F j Y H:i:s", PersianDateNumberCharacter.FARSI), oldMessage.message);
                                        } else {
